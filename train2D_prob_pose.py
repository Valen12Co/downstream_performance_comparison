...
training_methods = ['TIC']


class Train(object):
    def __init__(self, sampler: H36_dataset_probabilistic_pose, models: tuple, conf: ParseConfig,
    ...
    def train_model(self) -> dict:
        ...
        for e in range(self.epoch):
            ...
            for i, (gt, images, heatmaps, _,_,_) in tqdm(enumerate(self.torch_dataloader), ascii=True):
                #keypoints_2d, frame,heatmaps, subject, action, frame_num
    ...
    def covariance_estimation(self, aux_net: Union[AuxNet_HG, AuxNet_ViTPose],
        ...
        if name == 'MSE':
            ...
            loss = tic_loss(means, matrix, out_dim, pose_net, pose_encodings,
                            self.conf.use_hessian, self.conf.model_prob_pose, imgs,ground_truth)
...
def init_models(conf: ParseConfig) -> tuple:
    ...
    if conf.model_prob_pose == 'ViTPose':
        ...
        assert conf.model_prob_pose == 'Hourglass', "Choose either Hourglass, ViTPose"
...
def main() -> None:
    ...
    for trial in range(trials):
        ...
        train_obj = Train(train_dataset, (pose_model, aux_net, jnt_net), conf, training_pkg, trial)
        training_pkg = train_obj.train_model()


        with torch.no_grad():
            eval_obj = Evaluate(
                sampler=test_dataset, conf=conf, training_pkg=training_pkg, trial=trial)
